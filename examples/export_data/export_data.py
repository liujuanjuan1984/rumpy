...
def _trans(one):
    ...
    if t in _info:
        ...
        note = f'{note}<font color="darkred">点{_info[t]}给 `{name}` 发布的内容。</font>\n\n'
        ...
        note = f'{note}<font color="darkred">修改了个人信息。</font>\n\n'
    elif t == "annouce":
        note = f'{note}<font color="darkred">处理了链上请求。</font>\n\n'
    elif t == "reply":
        note = f'{note}<font color="darkred">发布了回复：</font>\n\n'
        if "text" in one:
        ...
        note = f'{note}<font color="darkred">发布了内容：</font>\n\n'
        if "text" in one:
            note = f'{note}{one["text"]}'
        if "imgs" in one:
            note = _save_img(one["imgs"], note)
